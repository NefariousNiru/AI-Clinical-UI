...
export function parseCsvToStudents(text: string): { students: NewRosterStudent[]; error?: string } {
	...
	if (students.length === 0) {
		return {
			students: [],
			error: "No valid rows found. Expected: name,email with @uga.edu emails.",
		};
	}
 ...
 * numeric paren bullets: 1) 2) 10)
 * common unicode bullets: • · ● ◦ ‣ ▪ ▫
 * Splits only when the marker is at start OR preceded by whitespace,
 * and followed by whitespace + content.
 * @param s the passed string
 */
export function splitInlineBullets(s: string): string {
	if (!s) return s;
	const re = /(^|[^\S\n])((?:\d{1,3}\)|[•·●◦‣▪▫]))(?=\s+\S)/g;
	...
	return s
		.replace(/\r\n/g, "\n")
		.replace(/\r/g, "\n")
		.replace(re, (_m, pre, tok, offset, whole) => {
			const prev = offset > 0 ? whole[offset - 1] : "";
			// keep bullets already at line start
			if (offset === 0 || prev === "\n") return `${pre}${tok}`;
			// otherwise start a new line at the bullet
			return `\n${tok}`;
		})
		.replace(/[ \t]+\n/g, "\n")
		.replace(/\n[ \t]+/g, "\n")
