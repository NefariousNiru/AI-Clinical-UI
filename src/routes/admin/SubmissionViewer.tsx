      ...
      <div className="absolute inset-x-0 top-16 mx-auto w-[min(1000px,95vw)] rounded-lg bg-white shadow-xl">
        <div className="flex items-center justify-between border-b px-5 py-3">
          <div className="text-sm font-semibold">
            Submission #{submission.id} {submission.synthetic ? "(synthetic)" : ""}
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="inline-flex rounded-md border border-gray-300 overflow-hidden">
              <button
                ...
                className={[
                  "px-3 py-1.5 text-xs",
                  view === "formatted" ? "bg-gray-900 text-white" : "bg-white text-gray-700 hover:bg-gray-50"
                ].join(" ")}
              >
                Formatted
              </button>
              <button
                ...
                className={[
                  ...
                  view === "json" ? "bg-gray-900 text-white" : "bg-white text-gray-700 hover:bg-gray-50"
                ].join(" ")}
              >
                JSON
              </button>
        ...
        <div className="px-5 py-4 space-y-4">
          {view === "json" ? (
            <JsonBlock data={submission} filename={`submission-${submission.id}.json`} />
          ) : (
            <>
              <div className="text-sm text-gray-600">
                Problems:{" "}
                <span className="font-medium text-gray-900">{submission.problems?.length ?? 0}</span>
              </div>
              ...
              {submission.problems?.length ? (
                  ...
                  {submission.problems.map((p, idx) => (
                    <div key={idx} className="rounded-lg border border-gray-200 overflow-hidden">
                      <div className="flex items-center justify-between border-b bg-gray-50 px-4 py-2">
                        <div className="text-sm font-semibold text-gray-800">{titleize(p.name)}</div>
                        {p.isPriority && (
                      ...
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                        <TextSection label="Identification" value={p.identification} />
                        <TextSection label="Explanation" value={p.explanation} />
                        <TextSection label="Plan & Recommendation" value={p.planRecommendation} />
                ...
                <div className="text-sm text-gray-500">No problems in this submission.</div>
              )}
            </>
          )}
        </div>
