...
export default function OutputPanel({ data, message, student }: OutputPanelProps) {
	const hasData = Array.isArray(data) && data.length > 0;
	const fallbackMessage = (message ?? "").trim() || "Output will appear here after processing…";
	return (
		<section
		...
		>
			...
			>
				{hasData ? (
					<ProblemFeedbackView data={data} studentProblems={student?.problems ?? null} />
				) : (
					<p className="text-sm text-primary whitespace-pre-wrap">{fallbackMessage}</p>
				)}
			</div>
		</section>
	);
