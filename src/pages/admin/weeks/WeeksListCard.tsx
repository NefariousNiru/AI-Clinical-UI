			...
			<div className="flex items-start justify-between gap-3">
				<div>
					...
					<p className="text-sm text-muted">
						Get week status, view past weeks, edit future weeks, or add a new one.
					</p>
				</div>
				...
				<button
					...
					title={
						canAdd
							? "Add a new week"
							: "Add Week is only allowed for the current semester."
					}
				>
					Add Week
				</button>
			...
			<div className="mt-4 border border-subtle rounded-2xl overflow-hidden">
				...
				{loading ? (
					...
					<div className="px-3 py-3 text-sm text-muted">
						No weeks found for this semester.
					</div>
				) : (
					<div className="divide-y divide-[var(--color-border-subtle)]">
						{weeks.map((w) => {
							...
							const patientDisplay =
								`${w.patientFirstName} ${w.patientLastName}`.trim();
							...
							return (
								...
								>
									...
									<div className="col-span-2 text-sm text-muted">
										{unixToIsoDate(w.start)}
									</div>
									<div className="col-span-2 text-sm text-muted">
										{unixToIsoDate(w.end)}
									</div>
									<div className="col-span-2 text-sm text-muted">
										<WorkupStatusPill status={w.status} />
									</div>
									<div className="col-span-1 text-sm text-accent font-medium text-right">
										{" "}
										{actionLabel}
									</div>
								</button>
