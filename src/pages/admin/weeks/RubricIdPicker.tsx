...
export default function RubricIdPicker({value, onChange, disabled = false}: Props) {
    const {ids, loading, error, canPrev, canNext, prev, next, pageLabel} = useRubricIdPager(20);
        ...
        <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-3">
                    ...
                    >
                        <option value="">{loading ? "Loading..." : "Select rubric"}</option>
                        {options.map((x) => (
                    ...
                    <button
                        ...
                        className={[
                            ...
                            disabled ? "opacity-50 cursor-not-allowed" : "btn-hover",
                        ].join(" ")}
                        onClick={addSelected}
            ...
            <div className="flex flex-wrap gap-2">
                {value.length === 0 ? (
                    <span className="text-sm text-muted">No rubrics selected.</span>
                ) : (
                    value.map((x) => (
                        <span
                            key={x}
