...
export default function TestsPage() {
	...
	const { sending, feedback, outputMsg, gradedSubmission, grade } = useTestGrader();
	...
	async function handleSend(): Promise<void> {
		const sub: TestSubmission | undefined =
			selectedId != null ? subs.find((s: TestSubmission) => s.id === selectedId) : undefined;
	...
	return (
		<div className="grid grid-cols-12 gap-8 px-4 lg:px-6 text-primary">
			...
			<section className="col-span-12 xl:col-span-10 space-y-6">
				<header className="space-y-3">
					...
					<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
						...
						<div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center md:justify-end">
							{/* Line 1: Model select */}
							<label className="flex w-full items-center gap-2 text-sm font-medium text-primary md:w-auto">
					...
					<div className="space-y-1">
						{loadingUi && <p className="text-sm text-muted">Loading configuration…</p>}
				...
				<OutputPanel data={feedback} message={outputMsg} student={gradedSubmission} />
			</section>

			{/* Sidebar */}
			<aside aria-label="Test Submissions list" className="col-span-12 xl:col-span-2 xl:pr-0">
				<div className="xl:sticky xl:top-16">
					<SubmissionList
						...
						onSelect={(id: unknown) => {
							...
							setSelectedId(Number.isFinite(numericId) ? numericId : null);
						}}
						...
						onView={(id: unknown) => {
							...
							const match = subs.find((x: TestSubmission) => x.id === numericId);
							if (match) {
								setViewerData(match);
								setViewerOpen(true);
							}
					...
					/>
					{loadingSubs && (
						<p className="px-3 py-2 text-xs text-muted">Loading submissions…</p>
					)}
				</div>
			</aside>
			...
			<LocalSessionsModal open={localModalOpen} onClose={() => setLocalModalOpen(false)} />
		</div>
	);
}
