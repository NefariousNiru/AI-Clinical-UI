 ...
 */
export default function SubmissionViewer({ open, onClose, submission }: SubmissionViewerProps) {
				...
				<>
					...
					{submission.tags.length > 0 && (
						<span className="text-xs text-muted">
							({submission.tags.map(titleizeDiseaseName).join(", ")})
						</span>
					)}
				</>
			}
			...
			{view === "json" ? (
				...
				<>
					<p className="mb-3 text-sm text-muted">
						Problems:{" "}
						<span className="font-medium text-primary">{problems.length}</span>
					</p>
					...
					{problems.length > 0 ? (
						...
						<p className="text-sm text-muted">No problems in this submission.</p>
					)}
				</>
			)}
		</Modal>
