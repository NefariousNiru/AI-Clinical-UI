...
import { isAdmin, useAdminMrpToolStatus } from "../hooks/settings";
...
export default function AdminSettingsModal(props: SettingsModalProps) {
	const { status, loading, error, toggle } = useAdminMrpToolStatus(props.open);

	function renderAdminExtras(profile: UserProfile | null): JSX.Element | null {
		if (!isAdmin(profile)) return null;

		const statusLabel =
			status === null ? (loading ? "Loading…" : "Unknown") : status === "on" ? "On" : "Off";
	...
	return <SettingsModalShared {...props} renderExtraSections={renderAdminExtras} />;
}
