...
export default function AccountActivationPage() {
    ...
    async function onSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        if (tokenMissing) {
            setErrors({
                form:
                    "Activation link is invalid or missing the token. Please request a new activation email.",
            });
            return;
        }
        ...
        if (!pwdResult.success) {
            newErrors.password =
                pwdResult.error.issues[0]?.message ?? "Invalid password.";
        }
        ...
        <div className="min-h-screen app-bg text-primary flex flex-col">
            ...
            >
                ...
                {tokenMissing && !success && (
                    <section
                        className="w-full max-w-[440px] rounded-2xl border border-danger bg-danger-soft text-danger px-5 py-4">
                        <p className="text-sm font-semibold mb-1">
                            Activation link is invalid
                        </p>
                        <p className="text-xs">
                            The activation link is missing a token or has been corrupted.
                ...
                {success && !tokenMissing && (
                    <section className="w-full max-w-[480px] rounded-2xl border border-accent bg-accent-soft px-5 py-4 text-sm text-primary">
                        ...
                        <p className="text-s mb-1">
                            Next steps: you will receive an onboarding email with details on how
                            to access the platform, course-specific instructions, and any
                            additional steps required by your instructor.
                        </p>
                        <p className="text-xs text-muted">
                            Keep an eye on your UGA inbox over the next few days. If you
                            do not see an email, check your spam folder or contact your
                            course coordinator.
                        </p>
                    </section>
                )}
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={[
                                            "w-full rounded-md border bg-input px-3 py-2 pr-10 text-base",
                                            "focus:outline-none focus:ring-1",
                                            errors.confirmPassword ? "border-danger" : "border-subtle",
                                        ].join(" ")}
