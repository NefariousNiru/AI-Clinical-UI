...
export default function Tabs({ value, onChange, items, fullWidth = false }: TabsProps) {
	const rootClass = [
		...
		<div className={rootClass} role="tablist" aria-label="View mode">
			{items.map((t, index) => {
				...
				const sizeLayoutClasses = fullWidth
					? "flex-1 min-w-0 py-1.5 text-xs font-medium text-center md:flex-none md:px-3"
					: "px-3 py-1.5 text-xs font-medium";

				const borderClass = index > 0 ? "border-l border-subtle" : "";
